package com.ohgiraffers.section02.encapualtaion.problem1;

public class Application {
    public static void main(String[] args) {

        /* 수업목표. 필드에 직접 접근하는 경우 발생할 수 있는 문제점을 이해할 수 있다. */
        /* 설명. 필드에 직접 접근하는 하는 경우 잘못된 데이터가 입력되어도 제어할 수 있는 장치가 없다.
            따라서, setter를 사용하여 Validation(유효성 검사)를 함으로써 잘못된 데이터가 설정되지 않도록 한다. */


        Monster monster1 = new Monster();  // 클래스를 도구로 사용하여 moster1이라는 이름과 hp를 가진 객체를 생성

        /* 설명. 1번 몬스터 생성 */
        monster1.name = "주황 버섯";
        monster1.hp = 200;

        /* 설명. 몬스터 정보 출력 */
        System.out.println("monster1의 이름: " + monster1.name);       // 참조연산자를 통한 값 출력
        System.out.println("monster1의 체력: " + monster1.hp);

        /* 설명. 2번 몬스터 생성(문제점 확인) */
        Monster monster2 = new Monster();
        monster2.name = "난폭한 주황버섯";
        monster2.hp = -300;                     // hp가 마이너스인 몬스터가 생성이됨

        System.out.println("monster2의 이름: " + monster2.name);
        System.out.println("monster2의 체력: " + monster2.hp);

        /* 설명. 3번 몬스터 생성(setter를 통한 접근 */
        Monster monster3 = new Monster();
        monster3.name = "초록버섯";
        monster3.setHp(350);                    // monster3의 hp 공간에 350이 할당된다.

        System.out.println("monster3의 이름: " + monster3.name);
        System.out.println("monster3의 체력: " + monster3.hp);

        /* 설명. 4번 몬스터 생성(setter를 통한 접근) */  //setter란 객체에서 변수값을 변경하거나 설정하는데 사용되는 메서드를 의미한다.
        Monster monster4 = new Monster();
        monster4.name = "날렵한 초록버섯";
        monster4.setHp(-100);

        System.out.println("monster4의 이름: " + monster4.name);
        System.out.println("monster4의 체력: " + monster4.hp);
    }
}
