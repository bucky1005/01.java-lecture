...
/* 필기. DTO는 Data Transfer Object의 약자로 객체를 생성할 때 데이터를 운반하는 역할을 한다. */
...
public class UserDTO {
    ...
    public UserDTO() {      // 모든 생성자는 기본 생성자를 가지고 있어야 한다.(없으면 에러남)
    }

    public UserDTO(String id, String pwd, String name, Date enrollDate) {       // 생성자
        this.id = id;                   // 현재 this는 생성자의 this 이므로
        this.pwd = pwd;
        this.name = name;
    ...
    public void setId(String id){ this.id = id; }  // 현재 this는 메소드의 this이므로 메소드를 호출한 객체를 의미함.
...
//    }
    ...
    @Override                           // Alt + Insert -> toStinrg() 선택
