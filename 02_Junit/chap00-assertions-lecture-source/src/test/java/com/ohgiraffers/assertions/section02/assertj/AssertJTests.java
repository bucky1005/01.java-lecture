...
public class AssertJTests {
    ...
    void testStringValidation() {
        ...
        // then
        /* 메모. assertJ에서 제공하는 assertThat을 사용하여 다양한 문자열 검사를 진행할 수 있다 */
    ...
    /* 목차 2. 숫자 테스트하기 */
    /* 필기. 숫자 또한 일치 여부가 아닌 범위값도 검증하는 단정문(메소드)를 통해 다양한 테스트를 수행할 수 있다. */
    @Test
    @DisplayName("숫자 테스트하기")
    void testIntegerValidation() {
        double p1 = Math.PI;

        Double actual = Double.valueOf(p1);
    ...
    void testLocalDateTimeValidation(){
        ...
        Assertions.assertThat(theDay)
                .hasYear(2014)
                .hasMonthValue(9)
                .hasMonth(Month.SEPTEMBER)          // Generic Type Month.SEPTEMBER
                .hasDayOfMonth(18)
    ...
    void testExceptionValidation()  {
        // 예외 생성 (실제 테스트에서는 예외가 발생한 받아와 검증을 진행)
        Throwable thrown = AssertionsForClassTypes.catchThrowable(() -> {
            throw new IllegalAccessException("잘못된 파라미터를 입력하셨습니다.");
        });
...
//        Member member3 = new Member(3, "user03", "이순신", 32);
//        Member member4 = new Member(4, "user04", "신사임당", 26);
...
//                .filteredOn(member -> member.getAge() > 20)     // members List에서 성인인 멤버 객체만 필터링
//                .containsOnly(member3, member4);                // 필터링된 객체들이 member3, member4가 맞으면 테스트 결과 true
//    }
    ...
    void testPropertyValidation()   {
        ...
        Assertions.assertThat(members)
                .filteredOn("age", 20)  // Member 객체의 age 필드가 20인 객체만 필터링
                .containsOnly(member1);                                 // 필터링된 객체가 member1이 맞는지 검증
    }
}
