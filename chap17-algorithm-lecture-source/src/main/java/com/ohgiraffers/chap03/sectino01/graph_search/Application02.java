...
/* 필기. 너비 우선 탐색(Breadth-First Search)
    선입 선출 구조에 queue를 활용한다.
...
public class Application02 {
    ...
    public static int solution(String input) throws IOException {
        ...
        for (int i = 0; i < K; i++) {
            ...
            /* 설명. 입력받은 x,y 좌표와 배열을 ㅏㅁㄴ들 때의 순서는 반대의 개념이다. */
            map[y][x] = 1;
        }
    ...
    private static void bfs(int x, int y) {
        ...
        /* 설명. 연속적으로 상하좌우에 배추가 심어져 있다면 반복한다.(기본) */
        while(!q.isEmpty()){
            Node node = q.poll();
