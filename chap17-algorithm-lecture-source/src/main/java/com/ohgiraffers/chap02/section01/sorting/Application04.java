...
/* 필기. 퀵 정렬(Quick Sort)
    ...
    시간복잡도는 O(nlogn)이지만 최악의 경우는 O(n^2)이 될 수도 있다.
 */
public class Application04 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < length; i++) {
            arr[i] = sc.nextInt();
            /* 메모. 첫 반복문에서 6 7 8 9와 같이 띄어쓰기로 값을 한번에 입력하면
                nextInt() 메소드는 개행 or 띄어쓰기 전까지의 값만 가져가기 때문에
                반복문이 돌면서 배열에 첫 반복문에서 입력한 값들이 차곡차곡 들어간다.
             */
        }
    ...
    public static void solution(int lo, int hi, int[] arr) {
        if(lo >= hi){
            return;     // low 인덱스가 high 인덱스 보다 커지면 solution 메소드 종료
        }
    ...
    private static int process(int left, int right, int[] arr) {
        ...
        while(true) {
            ...
            /* 설명. pivot 위치에 있는 값보다 크면서 lo 포인터의 위치보다 크거나 같은 위치이면 계속 줄인다. */
            do {
                hi --;
