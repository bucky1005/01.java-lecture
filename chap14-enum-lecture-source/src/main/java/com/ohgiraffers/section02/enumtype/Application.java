...
public class Application {
    public static void main(String[] args) {
        ...
        /* 설명 1. 열거 타입으로 선언된 인스턴스는 싱글톤으로 관리되며 인스턴스가 한 개 임을 보장한다.
        *   (순서 개념의 인덱스가 아니라 인스턴스임) */

        /* 추가. JAVA 필드와 HTML 필드에 Subjects 타입의 객체가 생성됨 -> 싱글톤 객체 */
        ...
        /* 설명 3. toString()/name()을 이용하여 필드명을 문자열로 변경하기 쉽다.(feat.wsitch 안씀) */
        System.out.println(Subjects.JAVA.toString());           // toString()은 오버라이드하여 출력 방식을 커스터마이징 가능
...
//        printSubjects(0);                 // Subjects 타입이 아니라서 에러 발생
    ...
    // 메소드의 매개변수가 Subjects 타입으로 자동 생성됨
