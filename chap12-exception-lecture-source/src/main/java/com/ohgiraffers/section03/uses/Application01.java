...
public class Application01 {
    public static void main(String[] args) {
        ...
        try {
            ...
            /* 필기.
                예외 처리 구문과 상관없이 반드시 수행해야 하는 경우 finally에 작성을 하며
                보통 사용한 자원을 반납할 목적으로 사용하게 된다.
             */
            try {
                /* 설명.
                    스트림이 생성되지 않았을 때 접근해서 close() 메소드를 호출하게 되면 NullPointerException이
                    발생할 수 있다. 따라서 NullPointException 방지를 위한 if문을 활용하게 된다.
                 */
                /* 추가. br은 file이 존재 할 때에 객체가 생성되므로 존재하지 않는 파일을 입력하면 객체가 생성되지 않음.
                    따라서 생성되지 않은 br을 close하려고 할 경우 예외가 발생 -> 예외처리 필수
                 */
                if(br != null)
                /* 설명
                *   입출력에서 사용산 스트림을 닫아주는 메소드이다. -> 자원을 반납할 목적
