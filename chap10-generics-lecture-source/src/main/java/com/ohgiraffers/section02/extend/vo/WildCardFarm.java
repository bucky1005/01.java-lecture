...
/* 필기. 와일드카드(wildcard)
    제네릭 클래스 타입의 객체를 메소드의 매개변수로 받을 때 타입 변수를 제안할 수 있다.
    <?>: 제한 없음
...
public class WildCardFarm {
    ...
    /* 설명. ?가 있는 경우 어떤 타입의 RabbitFarm(제네릭 타입 / Rabbit, Bunny, DrunkenBunny)이 와도 상관없다. */
    public void anyType(RabbitFarm<?> farm) {
        farm.getAnimal().cry();
    }

    /* 설명. RabbitFarm 중 에서도 Bunny 또는 Bunny 하위 타입이 있는 RabbitFarm만 가능 (Bunny 이거나 Rabbit 제네릭의 농장만 가능*/
    public void extendsType(RabbitFarm<? extends Bunny> farm){
        farm.getAnimal().cry();
    }

    /* 설명. RabbitFarm 중에서도 Bunny 상위 타입(=Rabbit)이 있는 RabbitFarm만 가능 */
